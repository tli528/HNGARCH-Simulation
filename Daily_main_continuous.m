...
% [row,column]=size(Week_r_end_mean);
% [row,column]=size(Hour_r_end_mean);  

% for i = 1:row-1
    for j =1:column

        %use given risk-free rate
        r_annual_percent=0.45067;
        r=(r_annual_percent/100)/252;

        log_real = log(Day_r_end(:,j));
        ...
        [x_max,f_val,flag,output] = fminunc(@(x) loss(x,r,h_init,log_ret_real,delta),x0,options);
%         [x_max,f_val,flag,output] = fminunc(@(x) loss(x,r,h_init,ret_sim),x0,options);
        % output.message

        %std error
        gradient = numfirstderiv(@(x) hn_lik_vec(inv_rescale(x),r,h_init,log_ret_real,delta),x_max,0.00001);
...
% return lik vec only
function [error,z,h] = hn_lik_vec(theta,r,h_init,ret_frequency,delta)
...
omega = theta(2) * delta;
alpha = theta(3) * delta;
beta = 1 - (1 - theta(4) - theta(3) * theta(5)^2) * delta - theta(3) * theta(5)^2;
gamma = theta(5) / sqrt(delta);
...
for i = 1:N
z(i) = (ret_frequency(i) - r * delta - lambda * h(i)) / sqrt(h(i));
h(i+1) = omega * delta + beta * h(i) + alpha * delta * (z(i) - gamma * sqrt(h(i) / delta))^2;
end
...
% optimize function
function error = loss(x,r,h_init,ret,delta)
vec_ret = hn_lik_vec(inv_rescale(x),r,h_init,ret,delta);
